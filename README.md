...
1.  **Clone the Repository**:
    ```bash
    git clone [https://github.com/versoindustries/HighNoonLLM.git](https://github.com/versoindustries/HighNoonLLM.git)
    cd highnoon-llm
    ```
...
## Project Structure
  ...
  * `Owasp/`: Folder containing OWASP guidelines and a preprocess script to convert the pdfs into `.txt` files. Will get turned into an OWASP dataset down the road. Initial plan is to use guidelines to instruct inference for coding.
